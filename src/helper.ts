...
export class SNSHelper extends BaseClass implements ISNSHelper {
  ...
  public async PublishAsync(
    ...
    // guard clauses
    if (this.ObjectOperations.IsNullOrWhitespace(topicArn)) {
      throw new Error(`[${action}]-Must supply topicArn`);
    }
    if (this.ObjectOperations.IsNullOrWhitespace(subject)) {
      throw new Error(`[${action}]-Must supply subject`);
    }
    if (this.ObjectOperations.IsNullOrWhitespace(message)) {
      throw new Error(`[${action}]-Must supply message`);
    }
